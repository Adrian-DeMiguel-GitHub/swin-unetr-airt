...
for split_idx, (train_files, test_files) in enumerate(train_test_splits_orginal_copy):
    ...
    plot_title_template = (
        f"Lowest Val Loss : {evaluation_summary['splits'][split_idx]['validation_loss_best_epoch']:.4f} - Lowest Epoch: {evaluation_summary['splits'][split_idx]['best_epoch']}\n"
        f"Number Training Patches: {evaluation_summary['splits'][split_idx]['num_train_patches']}\n"
    ...
    plot_title_template = (
        ...
        f"Number Testing Instances: {len(test_dataset)}\n"
...
ax2 = ax1.twinx()
ax2.plot(splits, validation_losses_splits, marker='s', color='blue', label='Validation Loss', zorder=3)
ax2.plot(splits, testing_losses_splits, marker='o', color='green', label='Testing Loss', zorder=3)
...
plot_title = (
    ...
    f"Avg. Test Loss: {avg_testing_loss:.4f} - Min/Max Difference Test Loss: {testing_loss_min_max_difference:.4f}\n"
