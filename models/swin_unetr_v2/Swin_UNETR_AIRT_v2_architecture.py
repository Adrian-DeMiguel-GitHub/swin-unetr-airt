...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.conv_in = nn.Conv3d(self.architecture_config["model_input_dimensions"][2], 1, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.conv_0 = nn.Conv3d(self.architecture_config["model_input_dimensions"][2]//patch_size[2], 1, kernel_size=1, stride=1, padding=0, bias=True)

        self.encoder_1 = UnetrBasicBlock(
            spatial_dims=2,
        ...
        self.conv_1 = nn.Conv3d((self.architecture_config["model_input_dimensions"][2]//patch_size[2])//2, 1, kernel_size=1, stride=1, padding=0, bias=True)

        self.encoder_2 = UnetrBasicBlock(
            spatial_dims=2,
        ...
        self.conv_2 = nn.Conv3d((self.architecture_config["model_input_dimensions"][2]//patch_size[2])//4, 1, kernel_size=1, stride=1, padding=0, bias=True)

        self.encoder_3 = UnetrBasicBlock(
            spatial_dims=2,
        ...
        self.conv_3 = nn.Conv3d((self.architecture_config["model_input_dimensions"][2]//patch_size[2])//8, 1, kernel_size=1, stride=1, padding=0, bias=True)

        self.encoder_4 = UnetrBasicBlock(
            spatial_dims=2,
        ...
        self.conv_4 = nn.Conv3d((self.architecture_config["model_input_dimensions"][2]//patch_size[2])//16, 1, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.decoder_0_in = UnetrUpBlock(
            spatial_dims=2,
            in_channels=feature_size,
            out_channels=feature_size,
            kernel_size=3,
    ...
    def forward(self, x_in):
        ...
        # Apply encoder blocks to extract features at multiple resolutions and then, apply 3D Convolution layer using a linear transformation across the depth dimension to aggregate its features 
        # In V1: enc0 = self.encoder1(x_in)
        enc_in = x_in.permute(0, 2, 1, 3, 4) # enc_in shape: batch_size, depth, channels, height, width
        enc_in = self.conv_in(enc_in) # enc_in shape: batch_size, 1, channels, height, width
        enc_in = enc_in.squeeze(1) # enc_in shape: batch_size, channels, height, width
        enc_in = self.encoder_in(enc_in) # enc_in shape: batch_size, channels, height, width
        
        
        # In V1: enc1 = self.encoder2(hidden_states_out[0])
        enc_0 = hidden_states_out[0].permute(0, 2, 1, 3, 4) # enc_0 shape: batch_size, depth/patch_size[2], channels, height/patch_size[0], width/patch_size[1] 
        enc_0 = self.conv_0(enc_0) # enc_0 shape: batch_size, 1, channels, height/patch_size[0], width/patch_size[1] 
        enc_0 = enc_0.squeeze(1) # enc_0 shape: batch_size, channels, height/patch_size[0], width/patch_size[1] 
        enc_0 = self.encoder_0(enc_0) # enc_0 shape: batch_size, channels, height/patch_size[0], width/patch_size[1]   
        
        
        # In V1: enc2 = self.encoder3(hidden_states_out[1])     
        enc_1 = hidden_states_out[1].permute(0, 2, 1, 3, 4) # enc_1 shape: batch_size, (depth/patch_size[2])/2, channels, (height/patch_size[0])/2, (width/patch_size[1])/2 
        enc_1 = self.conv_1(enc_1) # enc_1 shape: batch_size, 1, channels, (height/patch_size[0])/2, (width/patch_size[1])/2 
        enc_1 = enc_1.squeeze(1) # enc_1 shape: batch_size, channels, (height/patch_size[0])/2, (width/patch_size[1])/2
        enc_1 = self.encoder_1(enc_1) # enc_1 shape: batch_size, channels, (height/patch_size[0])/2, (width/patch_size[1])/2

        
        # In V1: enc3 = self.encoder4(hidden_states_out[2])    
        enc_2 = hidden_states_out[2].permute(0, 2, 1, 3, 4) # enc_2 shape: batch_size, (depth/patch_size[2])/4, channels, (height/patch_size[0])/4, (width/patch_size[1])/4 
        enc_2 = self.conv_2(enc_2) # enc_2 shape: batch_size, 1, channels, (height/patch_size[0])/4, (width/patch_size[1])/4 
        enc_2 = enc_2.squeeze(1) # enc_2 shape: batch_size, channels, (height/patch_size[0])/4, (width/patch_size[1])/4
        enc_2 = self.encoder_2(enc_2) # enc_2 shape: batch_size, channels, (height/patch_size[0])/4, (width/patch_size[1])/4

        
        # In V1: Did NOT exist   
        enc_3 = hidden_states_out[3].permute(0, 2, 1, 3, 4) # enc_3 shape: batch_size, (depth/patch_size[2])/8, channels, (height/patch_size[0])/8, (width/patch_size[1])/8 
        enc_3 = self.conv_3(enc_3) # enc_3 shape: batch_size, 1, channels, (height/patch_size[0])/8, (width/patch_size[1])/8 
        enc_3 = enc_3.squeeze(1) # enc_3 shape: batch_size, channels, (height/patch_size[0])/8, (width/patch_size[1])/8
        enc_3 = self.encoder_3(enc_3) # enc_3 shape: batch_size, channels, (height/patch_size[0])/8, (width/patch_size[1])/8

        
        # In V1: dec4 = self.encoder10(hidden_states_out[4])    
        enc_4 = hidden_states_out[4].permute(0, 2, 1, 3, 4) # enc_4 shape: batch_size, (depth/patch_size[2])/16, channels, (height/patch_size[0])/16, (width/patch_size[1])/16 
        enc_4 = self.conv_4(enc_4) # enc_4 shape: batch_size, 1, channels, (height/patch_size[0])/16, (width/patch_size[1])/16
        enc_4 = enc_4.squeeze(1) # enc_4 shape: batch_size, channels, (height/patch_size[0])/16, (width/patch_size[1])/16
        enc_4 = self.encoder_4(enc_4) # enc_4 shape: batch_size, channels, (height/patch_size[0])/16, (width/patch_size[1])/16

        # Apply the decoder blocks in a hierarchical manner to reconstruct the image
        
        # In V1: dec3 = self.decoder5(dec4, hidden_states_out[3])
        dec_4_3 = self.decoder_4_3(enc_4, enc_3)
        # In V1: dec2 = self.decoder4(dec3, enc3)
        dec_3_2 = self.decoder_3_2(dec_4_3, enc_2)
        # In V1: dec1 = self.decoder3(dec2, enc2)
        dec_2_1 = self.decoder_2_1(dec_3_2, enc_1)
        # In V1: dec0 = self.decoder2(dec1, enc1)
        dec_1_0 = self.decoder_1_0(dec_2_1, enc_0)
        # In V1: out = self.decoder1(dec0, enc0)
        out = self.decoder_0_in(dec_1_0, enc_in)

        
        # Generate final logit output applying head to the last upsampled decoder output
        logits = self.out(out)
...
class SwinUNETR_AIRT(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.model(x)
        # x shape: batch_size, out_classes, height, width
        return x
