...
from models.swin_unetr_v2 import Swin_UNETR_AIRT_v2_architecture
importlib.reload(Swin_UNETR_AIRT_v2_architecture)
from models.swin_unetr_v2.Swin_UNETR_AIRT_v2_architecture import SwinUNETR_AIRT
...
class SwinUNETR_AIRT_LightningModel(pl.LightningModule):
    ...
    def on_test_epoch_end(self):
        ...
        self.test_epoch_loss = np.mean(self.testing_batch_losses_in_epoch)
        self.testing_batch_losses_in_epoch.clear()

        self.test_epoch_mean_iou = np.mean(self.testing_batch_mean_ious_in_epoch)
        self.testing_batch_mean_ious_in_epoch.clear()

        self.test_epoch_dice_coeff = np.mean(self.testing_batch_dice_coeffs_in_epoch)
        self.testing_batch_dice_coeffs_in_epoch.clear()

        self.test_epoch_fpr = np.mean(self.testing_batch_fprs_in_epoch)
        ...
        if self.enable_testing_epoch_logging_into_console:
            ...
            print()
            print(f"==> (TESTING) Average Dice Loss (include_background=[{self.include_background_in_loss_and_metrics}]): {self.test_epoch_loss:.6f}")
            print()   
            
            print(f"==> (TESTING) Average Mean IoU (include_background=[{self.include_background_in_loss_and_metrics}]): {self.test_epoch_mean_iou:.6f}")
            print()
            
            print(f"==> (TESTING) Average Dice Coefficient (include_background=[{self.include_background_in_loss_and_metrics}]): {self.test_epoch_dice_coeff:.6f}")
            print()

            print(f"==> (TESTING) Average FPR (include_background=[{self.include_background_in_loss_and_metrics}]): {self.test_epoch_fpr:.6f}")
            print()
